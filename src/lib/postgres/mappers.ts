...
export function mapSubscription(row: SubscriptionRow): ChurchSubscription {
  return {
    ...
    providerStatus: optionalText(row.providerStatus),
    razorpaySubscriptionId: optionalText(row.razorpaySubscriptionId),
    razorpayPlanId: optionalText(row.razorpayPlanId),
    razorpayCustomerId: optionalText(row.razorpayCustomerId),
    canceledAt: row.canceledAt ? toMillis(row.canceledAt) : undefined,
