...
export function hasPlayableAudio(url: Quality | undefined | null): boolean {
  if (!url) return false;
...
export function getSongCoverUrl(imageUrl?: string | null): string {
  const trimmed = imageUrl?.trim();
  return trimmed || DEFAULT_SONG_COVER;
}
