...
export function HomeHeroSectionClient({ verse }: HomeHeroSectionClientProps) {
  ...
  return (
    <section
      ...
      className="relative overflow-hidden rounded-[16px] border border-[#1F1F1F]"
      style={{
        background:
          "radial-gradient(ellipse at 20% 50%, rgba(255,255,255,0.03) 0%, transparent 60%), linear-gradient(135deg, #0A0A0A 0%, #111111 50%, #0D0D0D 100%)",
      }}
    >
      <div className="relative grid grid-cols-1 md:grid-cols-2">
        <div className="flex flex-col justify-center px-5 py-5 sm:px-6 md:px-7 md:py-5">
          <p className="text-[10px] font-medium uppercase tracking-[2px] text-[#6B7280] md:text-[11px]">
            Faith Connect Hub
          </p>
          ...
          >
            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/[0.12] bg-white/[0.08] px-3 py-1 text-[11px] uppercase tracking-[0.16em] text-[#A1A1A1]">
            ...
            <h1
              ...
              className="mt-3 min-h-[3.9rem] max-w-[460px] text-[26px] font-bold leading-[1.18] text-white md:min-h-[4.8rem] md:text-[33px]"
            >
              {slide.heading}
            </h1>

            <p className="mt-2.5 min-h-[3.3rem] max-w-[420px] text-[14px] leading-[1.7] text-[#A1A1A1] md:text-[15px]">
              {slide.description}
            </p>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <Link
                ...
                className="inline-flex h-10 items-center rounded-lg bg-white px-5 text-sm font-semibold text-black transition-colors duration-200 hover:bg-[#E5E5E5] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"
              >
                {slide.primary.label}
              </Link>
              <Link
                href={slide.secondary.href}
                className="inline-flex h-10 items-center gap-1.5 text-sm text-[#A1A1A1] transition-colors duration-200 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"
              >
          ...
          >
            {SLIDES.map((item, index) => (
              <button
                ...
                onClick={() => goTo(index)}
                className={cn(
                  "h-[3px] rounded-[2px] transition-all duration-300 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40",
                  index === slideIndex
                    ? "w-6 bg-white"
                    : "w-2 bg-[#2A2A2A] hover:bg-[#4B4B4B]"
                )}
              />
            ))}
          </div>
        ...
        <div className="flex flex-col justify-center gap-2.5 border-t border-[#1F1F1F] px-5 py-5 sm:px-6 md:border-l md:border-t-0 md:px-7 md:py-5">
            ...
            {features.map((feature, slot) => {
              ...
              return (
                <Link
                  key={slot}
                  href={feature.href}
                  className="flex h-[98px] flex-col justify-center rounded-[10px] border border-white/[0.08] bg-white/[0.04] p-3.5 transition-colors duration-200 hover:border-white/[0.16] hover:bg-white/[0.08] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"
                >
                  <div
                    className={cn(
          ...
          <div className="min-h-[76px] rounded-[10px] border border-white/[0.07] bg-white/[0.03] px-3.5 py-2.5">
            <div className="flex items-center gap-2">
              <BookOpen className="size-3.5 shrink-0 text-[#6B7280]" aria-hidden />
              <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-[#6B7280]">
                Verse of the day
              </p>
              <span className="rounded px-2 py-0.5 text-[11px] text-white bg-white/[0.08]">
                {verse.reference}
              </span>
            </div>
            <blockquote className="mt-1 line-clamp-2 text-[13px] italic leading-[1.55] text-[#A1A1A1]">
