...
import { fetchGroupThread } from "@/lib/group-chat-client";
...
export function GroupChatThreadPanel({
  ...
  const query = useQuery({
    queryKey: ["group-chat-thread", groupId, rootId],
    enabled: Boolean(rootId),
    queryFn: async () => {
      const token = await getToken();
      if (!token) throw new Error("Sign in to view this thread.");
      return fetchGroupThread(groupId, rootId, token);
    },
  });
  ...
  return (
    <section
      aria-label="Thread"
      className="absolute inset-0 z-20 flex flex-col bg-white lg:inset-y-0 lg:left-auto lg:right-0 lg:w-[25rem] lg:border-l lg:border-[#E6E8EC] lg:shadow-[-12px_0_32px_-16px_rgba(15,23,42,0.18)]"
    >
      ...
      {canChat ?
        <div className="shrink-0 border-t border-[#E6E8EC] bg-[#F6F7FB] px-3 py-3">
